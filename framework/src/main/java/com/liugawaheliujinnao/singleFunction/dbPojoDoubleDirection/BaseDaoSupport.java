...
import com.liugawaheliujinnao.singleFunction.dbPojoDoubleDirection.Page;
import com.liugawaheliujinnao.singleFunction.dbPojoDoubleDirection.utils.BeanUtils;
import com.liugawaheliujinnao.singleFunction.dbPojoDoubleDirection.utils.DataUtils;
import com.liugawaheliujinnao.singleFunction.dbPojoDoubleDirection.utils.GenericsUtils;
...
/**
 * @Description:
 * BaseDao扩展类，主要功能是支持自动拼装sql语句，必须继承方可使用
 * 需要重写和实现以下三个方法
 * //设定主键列
...
public abstract class BaseDaoSupport<T extends Serializable, PK extends Serializable> {
    ...
    protected BaseDaoSupport() {
        try {
            ...
            op = new EntityOperation<T>(clazz, this.getPKColumn());
    ...
    private String makeSimpleReplaceSql(String tableName, Map<String, Object> params){
        ...
        int index = 0;
        for (String key : set) {
            sbKey.append(TableUtils.convertPropertyNameToColumnName(key));
