...
/**
 * @Description: 实体对象的反射操作，也不是单例 一个EntityOperation对应一个实体类
 * @Author: LiugawaHeLiujinnao
 * @Date: 2019-05-21
 */
public class EntityOperation<T> {
    private Logger log = Logger.getLogger(EntityOperation.class);
    //泛型实体Class对象，把这个实体类的字节码保存下来，避免每个方法都重复去获取class
    //这里又是一个小技巧，否则每次操作class的时候都需要通过classloader去加载，都是io操作
    //现在只需要获取一次，获取完之后保存起来
    public Class<T> entityClass = null;

    //也是在初始化的时候把所有的getter和setter保存到这个对象中来
    ...
    public EntityOperation(Class<T> clazz,String pk) throws Exception{
        ...
        String[] properties = this.allColumn.split(",");
        this.allColumn = "";
        for (String property : properties) {
            property = TableUtils.convertPropertyNameToColumnName(property);
            this.allColumn += property + ",";
        }
        this.allColumn = this.allColumn.substring(0, this.allColumn.length() - 1);
    ...
    //主要就是把查出来的结果和实体类中的列名一一对应上
    //真正的ORM的实现过程
    //前面做的一切的一切都是为了这一步，然后返回给用户
    //只不过是为了实现自动化，不写sql
