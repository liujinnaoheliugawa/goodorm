...
/**
 * @Description: 这只是一个工具类，这些功能是可以提供给用户直接用的
 * 为什么不设置成静态方法？要考虑到多线程环境
 * 加锁也是可以的
 * 但是现在从框架层面考虑这个问题，不是单例
 * 以简单化为主
...
public class QueryRuleSqlBuilder {
    ...
    private int CURR_INDEX = 0; //记录参数所在的位置，数组按顺序遍历
    ...
    //如果用户要自己执行SQL的时候，可以自由拼接
    //只拿到了QueryRule，我并不知道你执行的是CRUD中的哪一个
    //只是用来操作SQL语句的
    ...
    //"select * from t_member where name ? and addr = ? and age = ?"
    //匿名参数就是按数组顺序区分
    //"select * from t_member where name :name and addr = :addr and age = :age"
    //这时候就不管顺序了
    public Object[] getValues() {
        return this.valueArr;
    }

    //Java中默认的参数值列表与返回值无关，只与方法名和参数列表有关
    ...
    private void appendWhereSql(){
        ...
        //防止sql注入
        //比如and name = (select name from t_user where user = '')不允许
